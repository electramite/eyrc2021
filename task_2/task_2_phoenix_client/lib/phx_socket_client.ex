defmodule ToyRobot.PhoenixSocketClient do
  ...
  def connect_server do
    ...
    {:ok, response, channel} = PhoenixClient.Channel.join(socket, "robot:status")
    {:ok, response, channel}
  end
  ...
  def send_robot_status(channel, %ToyRobot.Position{x: x, y: y, facing: facing} = robot) do
    ...
    ## complete this funcion ##
    ###########################
    msg = %{x: robot.x, y: robot.y, facing: robot.facing}
    {:ok, status} = PhoenixClient.Channel.push(channel, "new_msg", msg)
    {:obstacle_presence, status}
  end
